...
typedef struct beacon_dwarf_cie_s {
    uint8_t pointerSize;
...
typedef struct beacon_dwarf_cfi_builder_s {
    beacon_DynArray_t buffer;
    ...
    bool isInPrologue;
} beacon_dwarf_cfi_builder_t;

typedef struct beacon_dwarf_lineProgramHeader_s {
    ...
    uint8_t lineRange;
    uint8_t opcodeBase;
}beacon_dwarf_lineProgramHeader_t;

typedef struct beacon_dwarf_lineProgramState_s {
    uint32_t regAddress;
    uint32_t regOpIndex;
    uint32_t regFile;
    uint32_t regLine;
...
}beacon_dwarf_lineProgramState_t;

typedef struct beacon_dwarf_debugInfo_builder_s {
    int version;
    int abbreviationCount;
    beacon_dwarf_lineProgramHeader_t lineProgramHeader;
    uint32_t lineHeaderLengthOffset;
    beacon_dwarf_lineProgramState_t lineProgramState;

    beacon_DynArray_t locationExpression;

    beacon_DynArray_t line;
    beacon_DynArray_t str;
    beacon_DynArray_t abbrev;
    beacon_DynArray_t info;

    beacon_DynArray_t lineTextAddresses;
    beacon_DynArray_t infoTextAddresses;
} beacon_dwarf_debugInfo_builder_t;
